 ...
 */
package cn.guoyukun.spring.shiro.filter;
...
/*        }
        ...
        if(user!=null){
            //把当前用户放到request中
            request.setAttribute(Constants.CURRENT_USER2, user);
            //druid监控需要
            ((HttpServletRequest)request).getSession().setAttribute(Constants.CURRENT_USERID, user.getId());
        }
*/
        return true;
    }
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception {
        /*
    	User user = (User) request.getAttribute(Constants.CURRENT_USER2);
        if (user == null) {
            return true;
        }
